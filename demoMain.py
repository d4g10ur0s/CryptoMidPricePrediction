...
def train_model(X_train, y_train, epochs, batch_size, learning_rate, momentum):
    ...
    early_stopping = EarlyStopping(monitor='loss', min_delta=5e-5, patience=1, verbose=1)
    # Train the model
...
def main():
    ...
    while 1 :
        ...
        if timeThreshold>60:
            ...
            dC.updateDataset(os.getcwd()+datasetName, os.getcwd()+processedDatasetName, symbol)
        # comute mse
        if not (len(lastPrediction)==0) :
            my_test = pd.DataFrame(data=a.loc[len(a)-1,["Mean price"]]).transpose()
            #mse = mean_absolute_error(lastPrediction[0],my_test)
        ...
        # predict or train
        if os.path.exists(os.getcwd()+modelName) and mse<5e-4:
            # make prediction
            ...
            if len(lastPrediction)==2:# last prediction is next prediction
